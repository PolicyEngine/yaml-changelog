...
""")
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as append_file:
                append_file.write("""
- bump: patch
  changes:
    fixed:
...
""")
    ...
    def test_missing_append_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
            ...
            try:
                with pytest.raises(FileNotFoundError, match="changelog_entry.yaml not found"):
...
""")
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as md:
                    ...
                    assert "[0.2.0]: https://github.com/TestOrg/test-repo/compare/0.1.0...0.2.0" in content
