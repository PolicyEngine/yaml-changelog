...
def find_version_patterns(previous_version: str, current_version: str) -> List[Tuple[str, str]]:
    ...
    patterns = [
        ...
        (rf'__version__\s*=\s*["\']({prev_escaped})["\']', f'__version__ = "{current_version}"'),
        (rf'__version__\s*=\s*["\']({prev_escaped})["\']', f"__version__ = '{current_version}'"),
        ...
        # Maven pom.xml
        (rf'<version>({prev_escaped})</version>', f'<version>{current_version}</version>'),
    ]
    
    return patterns


def bump_version_in_file(file_path: str, previous_version: str, current_version: str) -> bool:
...
def main():
    ...
    for file in args.files:
        if args.unsafe:
            ...
            if bump_version_in_file(file, changelog.previous_version, changelog.current_version):
